...
class ObjectDetector:
    """
    Object Detector สำหรับ Raspberry Pi
    รองรับ 3 รูปแบบ:
      1. PyTorch (.pt) - ใช้ทั่วไป
      2. NCNN (_ncnn folder) - เร็วที่สุดบน RPi (แนะนำ)
      3. TFLite (.tflite) - เหมาะถ้ามี Coral TPU
    
    ตัวอย่าง:
        # PyTorch model
        detector = ObjectDetector(weights="best.pt")
        
        # NCNN model (เร็วที่สุดบน Raspberry Pi)
        detector = ObjectDetector(weights="best_ncnn_model")
        
        # TFLite model (รองรับ int8 quantization)
        detector = ObjectDetector(weights="best_int8.tflite")
    """
    def __init__(self, weights="best.pt", cam_index=0, imgsz=480, conf_threshold=0.25, show_window=True, out_video="output_detect.mp4", fps=20.0, debug=False):
        self.weights = weights
        self.cam_index = cam_index
        self.imgsz = imgsz  # Default 480 สำหรับ Raspberry Pi
        self.conf_threshold = conf_threshold
        self.show_window = show_window
        self.out_video = out_video
        self.fps = fps
        self.debug = debug
        self.model_format = None  # เก็บ format ที่ใช้
    ...
    def _detect_model_format(self, weights_path):
        """ตรวจจับ format ของโมเดลอัตโนมัติ"""
        w = Path(weights_path)
        w_str = str(weights_path).lower()
        
        if w.is_dir() or '_ncnn' in w_str:
            return 'ncnn'
        elif w_str.endswith('.tflite'):
            return 'tflite'
        elif w_str.endswith('.pt'):
            return 'pytorch'
        else:
            # Default to pytorch
            return 'pytorch'
    ...
    def _load_model(self):
        w = Path(self.weights)
        self.model_format = self._detect_model_format(self.weights)
        
        format_names = {
            'ncnn': 'NCNN (เร็วสำหรับ ARM)',
            'tflite': 'TFLite (รองรับ int8)',
            'pytorch': 'PyTorch'
        }
        ...
        if self.model_format == 'ncnn':
            if not w.exists():
                print(f"[ERROR] ไม่พบโฟลเดอร์ NCNN model: {self.weights}")
                sys.exit(1)
        else:
            if not w.exists():
                print(f"[ERROR] ไม่พบไฟล์ model: {self.weights}")
                sys.exit(1)
        
        print(f"[*] โหลดโมเดล {format_names[self.model_format]} จาก {self.weights}")
